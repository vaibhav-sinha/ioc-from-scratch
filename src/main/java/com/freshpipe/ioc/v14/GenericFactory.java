...
public class GenericFactory {
    ...
    public static <T> InstanceRecord<T> create(Class<T> tClass, String name) throws Exception {
        ...
        if(objectFromConfig != null) {
            return new InstanceRecord<T>(wrapInProxy(objectFromConfig, tClass), name);
        }
        //Scan the entire project
        Reflections reflections = new Reflections("com.freshpipe.ioc.v14");
        ...
        return new InstanceRecord<T>(wrapInProxy(object, tClass), name);
    ...
    private static <T> T wrapInProxy(T obj, Class<T> tClass) {
        if(tClass.isAnnotationPresent(Audit.class)) {
            AuditInvocationHandler auditInvocationHandler = new AuditInvocationHandler(obj);
            return (T) Proxy.newProxyInstance(obj.getClass().getClassLoader(), new Class[]{tClass}, auditInvocationHandler);
        }
        return obj;
    }
